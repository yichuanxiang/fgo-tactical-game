...
const NobleCinematics = {
    ...
    playVideo(scene, charId, onComplete) {
        ...
        try {
            const W = scene.cameras.main.width, H = scene.cameras.main.height;
            const shade = scene.add.rectangle(W / 2, H / 2, W, H, 0x02040a, 0.92).setDepth(1999);
            const video = scene.add.video(W / 2, H / 2, videoKey).setDepth(2000);
            video.setDisplaySize(W, H);

            // 点击跳过
            const skipText = scene.add.text(W - 16, H - 14, 'SKIP', {
                fontSize: '12px',
                fill: '#b8c6e8',
            ...
            const skipHandler = () => {
                if (finished) return;
                finished = true;
                video.stop(); video.destroy(); shade.destroy(); skipText.destroy();
