class GameScene extends Phaser.Scene {
    ...
    createUnit(x, y, team, charId) {
        ...
        const isTenTurnLimit = charData.passive === 'tenTurnLimit';
        const baseHp = charData.hp || 40000;
        const baseNp = charData.np || 0;

        // 初始化技能冷却数组
        const skillCDs = charData.skills.map(() => 0);
        ...
        unit.data = {
            ...
            hp: baseHp,
            maxHp: hasUnlimitedPassive ? Infinity : baseHp,
            np: baseNp,
            maxNp: hasUnlimitedPassive ? Infinity : 100,
            shield: 0,
            diceCount: charData.diceCount || classData.diceCount,
        ...
        // 李存勖被动初始化
        if (isTenTurnLimit && typeof LiCunxuSkills !== 'undefined') {
            LiCunxuSkills.initPassive(unit);
            LiCunxuSkills.renderArrowMarkers(this, unit);
        }
    ...
    updateUnitBars(unit) {
        ...
        this.hpText.setText(`${fmtNum(hp)}${!isFinite(d.maxHp) ? '' : '/' + fmtNum(maxHp)}`);
    ...
    createDiceUI() {
        ...
        this.attackBtn = this.createStyledButton(sx + (btnW + gap), cy, '攻击', C.red, '#' + C.fg.toString(16).padStart(6, '0'), () => this.startAttackAction(), btnW, btnH);
        this.skillsBtn = this.createStyledButton(sx + (btnW + gap) * 2, cy, '技能', C.blue, '#' + C.fg.toString(16).padStart(6, '0'), () => this.showSkillSelection(false), btnW, btnH);
        this.nobleBtn = this.createStyledButton(sx + (btnW + gap) * 3, cy, '宝具', C.purple, '#' + C.fg.toString(16).padStart(6, '0'), () => this.useNoble(), btnW, btnH);
    ...
    updateActionButtons() {
        ...
        if (this.moveBtn) {
            if (d.hasMoved) {
                this.moveBtn.label.setStyle({ fill: '#555' });
            } else {
                this.moveBtn.label.setStyle({ fill: colorHex(C.fg) });
            }
        }
        // 攻击按钮：已行动则灰色
        if (this.attackBtn) {
            if (d.hasActed) {
                this.attackBtn.label.setStyle({ fill: '#555' });
            } else {
                this.attackBtn.label.setStyle({ fill: colorHex(C.fg) });
            }
        }
        // 技能按钮：已行动则灰色
        if (this.skillsBtn) {
            if (d.hasActed || d.silenced > 0) {
                this.skillsBtn.label.setStyle({ fill: '#555' });
            } else {
                this.skillsBtn.label.setStyle({ fill: colorHex(C.fg) });
            }
        }
    }

    startAttackAction() {
        if (!this.currentUnit || this.currentUnit.data.hasActed) return;
        if (this.waitingForAction) return;
        if (this.onlineMode && !this.isMyTurn()) return;

        this.actionText.setText('选择攻击目标');
    ...
    startMoveAction() {
        if (!this.currentUnit || this.currentUnit.data.hasMoved) return;
        if (this.waitingForAction) return;
        if (this.onlineMode && !this.isMyTurn()) return;

        // 检查定身状态（同时检查旧字段和Buff系统）
        if ((this.currentUnit.data.rooted && this.currentUnit.data.rooted > 0) || BuffSystem.isRooted(this.currentUnit)) {
            this.actionText.setText('被定身，无法移动!');
            this.addLog(`${this.currentUnit.data.name} 被定身，无法移动!`);
            return;
        }

        this.actionText.setText('选择移动位置（点击空地移动，点击其他位置取消）');
        this.showMoveRange(this.currentUnit, BuffSystem.getModifiedMoveRange(this.currentUnit));
    ...
    setupMoveInputAndFinish() {
        this.input.once('pointerdown', (pointer) => {
            ...
            if (moveTile) {
                ...
                // 移动完不结束回合，继续选择行动
                if (!this.currentUnit.data.hasActed) {
                    this.showUnitRanges(this.currentUnit);
                    this.actionText.setText('选择行动：攻击 / 技能 / 宝具 / 结束');
                } else {
                    this.finishAction();
                }
            } else {
                ...
                this.clearHighlights();
                this.showUnitRanges(this.currentUnit);
                this.actionText.setText('选择行动：移动 / 攻击 / 技能 / 宝具');
            }
        });
    }
    ...
    resetTestUnit() {
        ...
        const d = this.testUnit.data;
        const charHp = CHARACTERS[d.charId] ? CHARACTERS[d.charId].hp : 40000;
        d.hp = d.maxHp === Infinity ? 99999 : (d.maxHp || charHp);
        d.np = 100;
        d.shield = 0;
        d.atkBuff = 0;
        d.extraDice = 0;
        ...
        d.rooted = 0;
        d.burn = null;
        if (d.skillCDs) d.skillCDs = d.skillCDs.map(() => 0);
    ...
    selectUnit(unit) {
        ...
        const buffSummary = BuffSystem.getBuffSummary(unit);
        if (buffSummary) statusText += ' ' + buffSummary;

        const moveR = BuffSystem.getModifiedMoveRange(unit);
        const atkR = BuffSystem.getModifiedAttackRange(unit);
        this.unitInfoText.setText(
            `${d.name} [${d.className}] HP:${fmtNum(d.hp)}/${fmtNum(isFinite(d.maxHp) ? d.maxHp : Infinity)} NP:${d.np}/${isFinite(d.maxNp) ? d.maxNp : '∞'} 移动:${moveR} 射程:${atkR}${statusText}`
        );
        this.updateUnitBars(unit);
    }
    ...
    showUnitRanges(unit) {
        ...
        // 显示攻击范围（红色边框）
        let attackRange = BuffSystem.getModifiedAttackRange(unit);
    ...
    showSkillSelection(costHp) {
        ...
        // 检查沉默（旧字段 + Buff系统）
        if (this.currentUnit.data.silenced > 0 || BuffSystem.isSilenced(this.currentUnit)) {
        ...
        const cds = this.currentUnit.data.skillCDs || [];
        ...
        skills.forEach((skill, index) => {
            const cd = cds[index] || 0;
            const isSeal = sealed[index] && sealed[index] > 0;
            const available = cd <= 0 && !isSeal;
            const label = available ? skill.name : (isSeal ? `${skill.name}(封${sealed[index]})` : `${skill.name}(CD${cd})`);

            const btnX = centerX + (index - 1) * 150;
            const btn = this.createStyledButton(btnX, centerY, label, available ? colors[index] : 0x444444, available ? '#fff' : '#666', () => {
                if (!available) {
                    scene.actionText.setText(isSeal ? `${skill.name} 被封锁!` : `${skill.name} 冷却中(${cd}回合)`);
                    return;
                }
                if (costHp) {
                ...
                // 设置CD
                const cdValue = skill.cd || SKILL_CD_DEFAULT[index] || 3;
                scene.currentUnit.data.skillCDs[index] = cdValue;
        ...
        // 关闭按钮
        const closeBtn = this.createStyledButton(centerX, centerY + 50, '取消', 0x555555, '#aaa', () => {
            scene.clearSkillButtons();
            scene.showUnitRanges(scene.currentUnit);
            scene.actionText.setText('选择行动：移动 / 攻击 / 技能 / 宝具');
        }, 80, 30);
        closeBtn.setDepth(200);
        this.skillButtons.push(closeBtn);

        this.actionText.setText(costHp ? '选择技能（消耗15HP）' : '选择技能（带CD冷却）');
    }
    ...
    _onSkill(skill, index) {
        ...
        switch(skill.effect) {
            ...
            // 李存勖
            case 'pillowSwordBlood': LiCunxuSkills.usePillowSwordBlood(this, this.currentUnit, skill); break;
            case 'faceCrisis': LiCunxuSkills.useFaceCrisis(this, this.currentUnit, skill); break;
            case 'invincibleAdvance': LiCunxuSkills.useInvincibleAdvance(this, this.currentUnit, skill); break;
    ...
    _onNoble(noble) {
        ...
        switch(noble.effect) {
            ...
            case 'threeArrowsKingdom':
                LiCunxuSkills.useThreeArrowsKingdom(this, this.currentUnit, noble); break;
            case 'excalibur':
    ...
    useNoble() {
        ...
        if (this.currentUnit.data.hasActed) return;
        if (this.onlineMode && !this.isMyTurn()) return;
    ...
    attack(attacker, defender) {
        ...
        // 多乘区伤害公式
        const dmgResult = BuffSystem.calcDamage(attacker, defender, baseDamage, 'attack');
        let totalDamage = dmgResult.damage;
        if (dmgResult.log.length > 0) {
            this.addLog(`[乘区] ${dmgResult.log.join(' | ')}`);
        }
        ...
        this.tweens.add({
            ...
            onYoyo: () => {
                ...
                // 攻击命中NP充能（受NP获取率buff影响）
                this.gainNP(attacker, NP_GAIN.onAttackHit, '攻击命中');

                // 攻击后效果：吸血/反伤/追击/NP率加成
                const postFx = BuffSystem.getPostAttackEffects(attacker, defender, totalDamage);
                postFx.forEach(fx => {
                    if (fx.type === 'heal' && attacker.data) {
                        const maxHp = attacker.data.maxHp === Infinity ? 9999 : (attacker.data.maxHp || 100);
                        attacker.data.hp = Math.min(maxHp, attacker.data.hp + fx.value);
                        this.updateUnitBars(attacker);
                        if (this.showHealNumber) this.showHealNumber(attacker, fx.value);
                        this.addLog(fx.label);
                    } else if (fx.type === 'damage' && attacker.data) {
                        attacker.data.hp -= fx.value;
                        this.updateUnitBars(attacker);
                        if (this.showDamageNumber) this.showDamageNumber(attacker, fx.value);
                        this.addLog(fx.label);
                    } else if (fx.type === 'np' && attacker.data) {
                        this.gainNP(attacker, fx.value, 'NP率');
                    } else if (fx.type === 'doubleStrike') {
                        this.addLog('追击发动!');
                    }
                });
                ...
                if (attacker.data && attacker.data.class === 'berserker') {
                    ...
                    attacker.data.np = Math.min(100, attacker.data.np + npGain);
                    attacker.data.berserkNpPending = true;
                    this.updateUnitBars(attacker);
    ...
    dealDamage(unit, damage, damageType = 'attack') {
        ...
        unit.data.hp -= damage;
        // 受击NP充能
        if (damage > 0) this.gainNP(unit, NP_GAIN.onTakeDamage, '受击');
    ...
    destroyUnit(unit) {
        ...
        if (unit.data && unit.data.isClone && !unit.data.isNobleClone && unit.data.cloneMaster && unit.data.cloneMaster.data) {
            ...
            if (transferNp > 0) {
                ...
                this.addLog(`月影被击杀！NP(${transferNp})转移至本体`);
            } else {
                this.addLog('月影被击杀！');
            }

            // 月影死亡爆炸：对周围敌方造成伤害（走乘区，吃本体施加的易伤）
            if (unit.data.deathExplodeMin) {
                const cx = unit.data.x, cy = unit.data.y;
                const master = unit.data.cloneMaster;
                const explodeDmg = Phaser.Math.Between(unit.data.deathExplodeMin, unit.data.deathExplodeMax);
                if (this.createRadialBurstAt) this.createRadialBurstAt(unit.x, unit.y, 0xb8a9d4, 90, 16, 1400);
                if (this.shakeCamera) this.shakeCamera(120, 0.004);
                this.units.filter(u => u.data && u.data.team !== unit.data.team && !u.data.isClone && !u.data.isNobleClone).forEach(e => {
                    const dist = Math.abs(e.data.x - cx) + Math.abs(e.data.y - cy);
                    if (dist <= 2) {
                        const attacker = (master && master.data) ? master : unit;
                        const res = BuffSystem.calcDamage(attacker, e, Math.round(explodeDmg / DAMAGE_SCALE), 'skill');
                        this.dealDamage(e, res.damage, 'skill');
                        if (this.showDamageNumber) this.showDamageNumber(e, res.damage);
                        this.addLog(`月影爆裂! ${e.data.name} 受到${fmtNum(res.damage)}伤害`);
                    }
                });
            }
        }
    ...
    finishAction() {
        ...
        // 标记行动已完成
        if (this.currentUnit) {
            this.currentUnit.data.hasActed = true;
        }
        ...
        if (this.currentUnit && this.currentUnit.data.dogLightActions > 0) {
            ...
            if (this.currentUnit.data.dogLightActions >= 0) {
                this.currentUnit.data.hasActed = false;
                if (this.currentUnit.data.dogLightActions > 0) {
        ...
        if (this.currentUnit && this.currentUnit.data.extraActionNoSkill) {
            ...
            this.currentUnit.data.noSkillThisTurn = true;
            this.currentUnit.data.hasActed = false;
    ...
    // Buff系统：回合开始处理
    processBuffTick(team) {
        const units = this.units.filter(u => u.data && u.data.team === team);
        const scene = this;
        units.forEach(unit => {
            const logs = BuffSystem.tickBuffs(unit, scene);
            if (logs) logs.forEach(msg => scene.addLog(msg));
            BuffSystem.renderBuffIcons(scene, unit);
            // 李存勖被动 tick
            if (unit.data.charId === 'archer_licunxu' && typeof LiCunxuSkills !== 'undefined') {
                LiCunxuSkills.tickPassive(unit, scene);
                LiCunxuSkills.renderArrowMarkers(scene, unit);
            }
        });
    }
    ...
    refreshAllBuffIcons() {
        this.units.forEach(unit => {
            if (unit && unit.data) BuffSystem.renderBuffIcons(this, unit);
        });
    }

    // 技能冷却递减
    tickSkillCDs(unit) {
        if (!unit || !unit.data || !unit.data.skillCDs) return;
        for (let i = 0; i < unit.data.skillCDs.length; i++) {
            if (unit.data.skillCDs[i] > 0) unit.data.skillCDs[i]--;
        }
        // 封锁技能递减
        if (unit.data.sealedSkills) {
            Object.keys(unit.data.sealedSkills).forEach(key => {
                if (unit.data.sealedSkills[key] > 0) unit.data.sealedSkills[key]--;
            });
        }
    }

    // NP充能（支持区间随机）
    gainNP(unit, rangeOrAmount, source) {
        if (!unit || !unit.data) return;
        const amount = typeof rangeOrAmount === 'number' ? rangeOrAmount : rollNPGain(rangeOrAmount);
        const maxNp = unit.data.maxNp || 100;
        const before = unit.data.np;
        if (maxNp === Infinity) {
            unit.data.np += amount;
        } else {
            unit.data.np = Math.min(maxNp, unit.data.np + amount);
        }
        const gained = unit.data.np - before;
        if (gained > 0) {
            this.updateUnitBars(unit);
            this.updateNobleButton();
            if (source) {
                this.addLog(`${unit.data.name} ${source}: NP+${gained}`);
                if (this.showCombatText) this.showCombatText(unit, `NP+${gained}`, '#f1c40f', '11px');
            }
        }
    }
